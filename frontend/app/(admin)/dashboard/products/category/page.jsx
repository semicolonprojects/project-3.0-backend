...
const Page = () => {
  ...
  useEffect(
    () => {
      const fetchCategoryProducts = async () => {
        try {
          const categoriesData = await getCategoryProducts(currentPages);
  ...
  const handleDeleteTask = async (productCategoryId) => {
    ...
    try {
      ...
      setCategories(
        categories.filter((category) => category.id !== productCategoryId)
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (categoryId) {
        ...
        toast.dismiss();
        toast.success(`Category updated successfully: ${res.data.name}`, {
          position: "bottom-right",
        });
      } else {
        ...
        toast.dismiss();
        toast.success(`Category created successfully: ${res.data.name}`, {
          position: "bottom-right",
        });
      }
      router.refresh();
          ...
          <div className="flex-row items-center justify-between p-4 space-y-3 sm:flex sm:space-y-4 sm:space-x-4">
            <div className="">
              ...
              <p className="mt-1 text-sm font-normal text-pretty text-gray-500 ">
                Berisi List-list product category yang ditunjukkan di dalam
                company profile, anda dapat menambah, merubah dan menghapus
                product yang akan ditampilkan
              </p>
            </div>
          </div>
          <div className="flex flex-col items-center px-3 justify-between space-y-3 md:flex-row md:space-y-0 md:space-x-5">
            ...
            <div className="flex flex-col items-stretch justify-end flex-shrink-0 w-full space-y-2 md:w-auto md:flex-row md:space-y-0 space-x-4">
              ...
              >
                ...
                <span className="text-sm">Add New Product Category</span>
              </button>
            </div>
          </div>
        </caption>
            ...
            >
              ...
              <td className="px-1 py-3 text-right">
                <div className="flex flex-row gap-3 justify-center items-center">
                  ...
                  <button
                    className="grid grid-flow-row text-gray-600"
                    onClick={() => handleDeleteTask(category.id)}
                  >
                    <svg
