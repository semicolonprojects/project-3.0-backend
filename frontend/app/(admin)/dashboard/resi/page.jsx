...
const Resi = () => {
  ...
  useEffect(() => {
    const fetchResis = async () => {
      try {
        const resisData = await getResis(currentPages);
  ...
  const filteredResis = resis.filter(
    (resi) =>
      resi.nama_pelanggan.toLowerCase().includes(search.toLowerCase()) ||
      resi.kode_resi.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const handleDeleteTask = async (id) => {
    ...
    try {
      await deleteResi(id);
      setResis(resis.filter((resi) => resi.kode_resi !== id));
      toast.success("Resi deleted successfully");
      router.refresh();
    } catch (error) {
              ...
              <div className="flex-row items-center justify-between p-4 space-y-3 sm:flex sm:space-y-4 sm:space-x-4">
                <div className="">
                  ...
                  <p className="mt-1 text-sm font-normal text-pretty text-gray-500 ">
                    Berisi List-list resi yang ditunjukkan di dalam company
            ...
            <tbody>
              {filteredResis.map((resi) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">{resi.nama_pelanggan}</td>
                  <td className="px-6 py-4">{resi.status_pengerjaan}</td>
                  <td className="px-6 py-4">{resi.category}</td>
                  <td className="px-1 py-3 text-right">
                    <div className="grid grid-flow-col gap-1">
                      <Link href={`./resi/${resi.kode_resi}/show`}>
                        <button
                          className={`grid grid-flow-row text-gray-600 ${
                            resi.status_pengerjaan === "Selesai" ? "hidden" : ""
                          }`}
                        >
                          <svg
                      ...
                      <button
                        className="grid grid-flow-row text-gray-600"
                        onClick={() => handleDeleteTask(resi.kode_resi)}
                      >
                        <svg
                          className="w-6 h-6 ml-2"
                          data-slot="icon"
